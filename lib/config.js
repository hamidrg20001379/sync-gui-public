...
export const statePath = path.join(path.dirname(configPath), '.sync-state.json');
...
function migrateFromFlat(config) {
  ...
  const remoteKey = r => JSON.stringify({
    kind: r.kind,
    host: r.host,
    port: r.port,
    username: r.username,
    authMethod: r.authMethod || (r.privateKeyPath ? 'key' : 'password'),
...
function migrateFromOld(config) {
  ...
  for (const project of config.projects || []) {
    ...
    const connection = remote ? {
      ...
      authMethod: remote.authMethod || (remote.privateKeyPath ? 'key' : 'password'),
      privateKeyPath: remote.privateKeyPath || '', keyPassphrase: remote.keyPassphrase || ''
    } : { kind: 'local' };
