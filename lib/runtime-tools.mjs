...
export function toBashPath(value, baseDir = process.cwd()) {
  const normalized = path.resolve(value).replace(/\\/g, '/');
  const match = normalized.match(/^([A-Za-z]):\/(.*)$/);
  if (!match) return normalized;

  const drive = match[1].toLowerCase();
  const prefix = fs.existsSync(bundledWindowsBash(baseDir)) ? '/cygdrive/' : '/';
  return `${prefix}${drive}/${match[2]}`;
}
...
export function spawnRuntimeBash(command, extraEnv = {}, baseDir = process.cwd()) {
  const bash = resolveBashPath(baseDir);
  const env = runtimeToolEnv(extraEnv, baseDir);
  const options = { cwd: baseDir, env, windowsHide: true };
  if (process.platform !== 'win32') return spawn(bash, ['-lc', command], options);

  // ponytail: PowerShell owns one hidden console so Bash and its native children do not flash separate windows.
  ...
  const script = [
    `$bash=${powershellQuote(bash)}`,
    `$command=${powershellQuote(command)}`,
    `& $bash '--noprofile' '--norc' '-lc' $command`,
    'exit $LASTEXITCODE',
  ].join('; ');
  const encoded = Buffer.from(script, 'utf16le').toString('base64');
  return spawn(powershell, ['-NoProfile', '-NonInteractive', '-WindowStyle', 'Hidden', '-EncodedCommand', encoded], options);
}
...
export function runtimeToolEnv(extraEnv = {}, baseDir = process.cwd()) {
  ...
  return {
    ...
    TMP: tmpDir,
    TEMP: tmpDir,
    HOME: toBashPath(homeDir, baseDir),
  };
}
