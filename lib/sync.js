...
import { prepareSshAuth, sshExecutable, sshInvocation, sshShellPathPrefix, usesNativeWindowsOpenSsh } from './ssh.js';
...
async function syncSsh({ item, target, remote, direction, dryRun, noDelete, restrictToChangedFiles, categoryValues }) {
  ...
  if (usesNativeWindowsOpenSsh()) {
    return syncSshScp({ item, source, dest, remote, direction, dryRun });
  }
  const remoteSpec = `${remote.username}@${remote.host}:${remotePath}`;
  const ssh = sshInvocation() + ' -p ' + shq(String(remote.port || 22)) + ' -o StrictHostKeyChecking=accept-new' + sshKnownHostsOption();
...
async function listRemoteMatches(remote, pattern) {
  ...
  const ssh = sshInvocation() + ' -p ' + shq(String(remote.port || 22)) + ' -o StrictHostKeyChecking=accept-new' + sshKnownHostsOption() + ' ' + shq(remote.username + '@' + remote.host) + ' ' + shq(findCommand);
...
async function syncSshExactDown({ item, remote, remotePath, localPath, noDelete, restrictToChangedFiles }) {
  if (usesNativeWindowsOpenSsh()) {
    return syncSshScp({ item, source: remotePath, dest: localPath, remote, direction: 'down', dryRun: false });
  }
  const remoteSpec = `${remote.username}@${remote.host}:${remotePath}`;
  const ssh = sshInvocation() + ' -p ' + shq(String(remote.port || 22)) + ' -o StrictHostKeyChecking=accept-new' + sshKnownHostsOption();
  const flags = ['-az', '--human-readable', '--itemize-changes', '--no-o', '--no-g'];
...
async function syncSshScp({ item, source, dest, remote, direction, dryRun }) {
  if (dryRun) return { code: 0, output: 'Would transfer with native scp.' };

  const scp = nativeScpExecutable();
  if (!scp) return { code: 1, output: 'Windows OpenSSH scp.exe was not found.' };

  const remoteSpec = `${remote.username}@${remote.host}:${direction === 'up' ? dest : source}`;
  if (direction === 'up') {
    const remoteDir = item.type === 'folder' ? dest.replace(/\/+$/, '') : path.posix.dirname(dest);
    const mkdir = await runBash(
      `${sshInvocation()} -p ${shq(String(remote.port || 22))} -o StrictHostKeyChecking=accept-new${sshKnownHostsOption()} ${shq(`${remote.username}@${remote.host}`)} ${shq(`mkdir -p -- ${shq(remoteDir)}`)}`,
      remote.password,
    );
    if (mkdir.code !== 0) return mkdir;
  } else {
    await fsp.mkdir(item.type === 'folder' ? dest : path.dirname(dest), { recursive: true });
  }

  const sourcePath = direction === 'up'
    ? (item.type === 'folder' ? path.join(source, '.') : source)
    : remoteSpec;
  const destinationPath = direction === 'up'
    ? remoteSpec
    : (item.type === 'folder' ? path.join(dest, '.') : dest);
  const args = [
    ...(item.type === 'folder' ? ['-r'] : []),
    '-P', String(remote.port || 22),
    '-o', 'StrictHostKeyChecking=accept-new',
    ...(process.env.SYNC_GUI_KNOWN_HOSTS ? ['-o', `UserKnownHostsFile=${process.env.SYNC_GUI_KNOWN_HOSTS}`] : []),
    sourcePath,
    destinationPath,
  ];
  const result = await runNativeScp(scp, args, remote.password);
  if (result.code === 0) result.output = `Transferred with native scp.\n${result.output}`.trim();
  return result;
}

function nativeScpExecutable() {
  if (process.platform !== 'win32') return null;
  const ssh = toWindowsPath(sshExecutable());
  return /\\ssh\.exe$/i.test(ssh) ? path.join(path.dirname(ssh), 'scp.exe') : null;
}

function toWindowsPath(value) {
  const normalized = String(value).replace(/\\/g, '/');
  const match = normalized.match(/^(?:\/cygdrive)?\/([A-Za-z])\/(.*)$/) || normalized.match(/^([A-Za-z]):\/(.*)$/);
  if (!match) return value;
  return `${match[1].toUpperCase()}:\\${match[2].replace(/\//g, '\\')}`;
}

async function runNativeScp(executable, args, password) {
  const auth = await prepareSshAuth(password, { direct: true });
  return new Promise(resolve => {
    const child = spawn(executable, args, {
      cwd: process.cwd(),
      env: { ...process.env, ...auth.env, SSHPASS: password || '' },
      windowsHide: true,
    });
    let output = '';
    let settled = false;
...
export async function runBashProcess(command, password) {
  const auth = await prepareSshAuth(password);
  try {
  return new Promise(resolve => {
