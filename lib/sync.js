...
export async function runSync({ direction = 'up', dryRun = false, noDelete = false, force = false, itemTargets = {} }) {
  const config = await readConfig();
  const restrictToChangedFiles = !force && Boolean(config.settings?.restrictToChangedFiles);
  ...
  for (const mapping of mappings) {
    ...
    if (direction === 'up' && !dryRun && result.code === 0 && String(target.postSyncCommand || '').trim()) {
      postUploadMappings.push(mapping);
    }
  ...
  if (finalCode === 0) {
    for (const mapping of postUploadMappings) {
      const { item, target, remote } = mapping;
      const label = `${target.name || `target ${mapping.targetIndex}`} / ${remote.name || remote.id || 'local'}`;
      chunks.push(`[${item.name} ? ${label}] after successful upload`);
      const result = await runPostUploadCommand(target, remote);
      if (result.output) chunks.push(result.output);
      if (result.code !== 0 && finalCode === 0) finalCode = result.code;
      chunks.push(`[${item.name} ? ${label}] after-upload exit ${result.code}`);
    }
  }
...
async function runPostUploadCommand(target, remote) {
  const command = String(target.postSyncCommand || '').trim();
  if (!shouldUseSsh(target, remote)) return runBash(command);
  const ssh = sshInvocation(sshAuthMethod(remote) !== 'key' && Boolean(remote.password), remote.privateKeyPath)
    + ' -p ' + shq(String(remote.port || 22))
    + ' -o StrictHostKeyChecking=accept-new' + sshKnownHostsOption();
  return runBash(`${ssh} ${shq(`${remote.username}@${remote.host}`)} ${shq(command)}`, remote.password, remote.keyPassphrase);
}
