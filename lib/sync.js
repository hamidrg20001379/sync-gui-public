...
import { prepareSshAuth, sshInvocation, sshShellPathPrefix } from './ssh.js';
...
async function syncSsh({ item, target, remote, direction, dryRun, noDelete, restrictToChangedFiles, categoryValues }) {
  ...
  const ssh = sshInvocation(Boolean(remote.password)) + ' -p ' + shq(String(remote.port || 22)) + ' -o StrictHostKeyChecking=accept-new' + sshKnownHostsOption();
  ...
  if (direction === 'up') {
    ...
    const src = item.type === 'folder' ? rsyncFolderContents(localSource) : localSource;
    ...
    const src = item.type === 'folder' ? rsyncFolderContents(remoteSpec) : remoteSpec;
...
async function listRemoteMatches(remote, pattern) {
  ...
  const ssh = sshInvocation(Boolean(remote.password)) + ' -p ' + shq(String(remote.port || 22)) + ' -o StrictHostKeyChecking=accept-new' + sshKnownHostsOption() + ' ' + shq(remote.username + '@' + remote.host) + ' ' + shq(findCommand);
...
export function rsyncFolderContents(value) {
