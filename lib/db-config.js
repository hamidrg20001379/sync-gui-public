  ...
  for (const [sortOrder, category] of categories.entries()) {
    ...
    await tx.category.create({
      data: {
        ...
        parentId,
        variables: JSON.stringify({ ...(category.variables || {}), id: category.id, label: category.label || category.id }),
        sortOrder
      }
    });
    ...
    for (const [mappingSortOrder, mapping] of (category.mappings || []).entries()) {
      await tx.mapping.create({
        data: {
          ...
          categoryId,
          templateId: mapping.templateId || templateId,
          type: mapping.type,
