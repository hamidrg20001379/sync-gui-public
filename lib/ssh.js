...
export function sshInvocation(usePassword = true) {
  ...
  const command = usesNativeWindowsOpenSsh(executable)
    ? usePassword ? 'env BASH_ENV=/tmp/sync-gui-bash-noop.sh bash.exe /tmp/sync-gui-ssh-wrapper.sh' : 'ssh'
    : usesWindowsOpenSsh(executable)
      ? usePassword ? 'env BASH_ENV=/tmp/sync-gui-bash-noop.sh bash.exe /tmp/sync-gui-ssh-wrapper.sh' : 'ssh'
      : executable === 'ssh' ? executable : shq(executable);
  // Linux containers have sshpass available; use it for configured passwords instead of opening an interactive prompt.
  return process.platform !== 'win32' && usePassword ? `sshpass -e ${command}` : command;
}
...
export async function prepareSshAuth(password, { direct = false } = {}) {
  ...
  const sshPath = usesNativeWindowsOpenSsh(sshExecutable())
    ? 'C:/Windows/System32/OpenSSH/ssh.exe'
    : shq(sshExecutable());
  await fsp.writeFile(wrapper, `MSYS_NO_PATHCONV=1 MSYS2_ARG_CONV_EXCL="*" BASH_ENV=/tmp/sync-gui-askpass.sh exec ${sshPath} "$@"\n`, 'utf8');
  await fsp.chmod(script, 0o755);
  return {
    // Native Windows OpenSSH needs an executable; bash reads BASH_ENV before the prompt argument.
    env: { SSH_ASKPASS: usesNativeWindowsOpenSsh(sshExecutable()) ? bash : toMsysPath(bash), SSH_ASKPASS_REQUIRE: 'force', DISPLAY: '1', ...(usesNativeWindowsOpenSsh(sshExecutable()) ? { MSYS_NO_PATHCONV: '1', MSYS2_ARG_CONV_EXCL: '*' } : {}) },
    cleanup: async () => {}
  };
}
