...
function bundledSsh() {
  ...
  candidates.push(path.join('C:\\msys64', 'usr', 'bin', 'ssh.exe'));
...
export function sshInvocation(usePassword = true, identityFile = '') {
  ...
  const identity = identityFile?.trim()
    ? `-i ${shq(sshIdentityPath(identityFile, executable))}`
    : '';
  // Bundled MSYS2 OpenSSH is a POSIX-style client and must use sshpass.
  // Only native Windows OpenSSH is handled through SSH_ASKPASS.
  const auth = usePassword && !usesNativeWindowsOpenSsh(executable) ? 'sshpass -e ' : '';
  return `${auth}${command}${identity ? ` ${identity}` : ''}`;
}

export function sshAuthMethod(remote) {
  return remote?.authMethod || (remote?.privateKeyPath ? 'key' : 'password');
}
...
export async function prepareSshAuth(password, { direct = false, passphrase = '' } = {}) {
  const secret = passphrase || password;
  const needsAskpass = Boolean(passphrase) || (Boolean(password) && usesNativeWindowsOpenSsh());
  if (!secret || !needsAskpass) return { env: {}, cleanup: async () => {} };
  ...
  if (direct) {
    ...
    await fsp.writeFile(script, '#!/usr/bin/env bash\nprintf "%s" "${SYNC_GUI_ASKPASS_SECRET:-${SSHPASS:-}}"\nexit 0\n', 'utf8');
    return {
      env: { SSH_ASKPASS: bash, BASH_ENV: script, SSH_ASKPASS_REQUIRE: 'force', DISPLAY: '1', SYNC_GUI_ASKPASS_SECRET: secret },
  ...
  if (usesNativeWindowsOpenSsh(sshExecutable())) {
    const askpass = path.join(directory, `sync-gui-askpass-${process.pid}-${Date.now()}.cmd`);
    await fsp.writeFile(
      askpass,
      '@echo off\r\npowershell.exe -NoProfile -NonInteractive -Command "[Console]::Write($env:SYNC_GUI_ASKPASS_SECRET)"\r\n',
      'utf8'
    );
    return {
      env: {
        SSH_ASKPASS: toMsysPath(askpass),
        SSH_ASKPASS_REQUIRE: 'force',
        DISPLAY: '1',
        MSYS_NO_PATHCONV: '1',
        MSYS2_ARG_CONV_EXCL: '*',
        SYNC_GUI_ASKPASS_SECRET: secret
      },
      cleanup: () => fsp.rm(askpass, { force: true })
    };
  }
  ...
  return {
    ...
    env: usesNativeWindowsOpenSsh(sshExecutable())
      ? { SSH_ASKPASS: bash, SSH_ASKPASS_REQUIRE: 'force', DISPLAY: '1', MSYS_NO_PATHCONV: '1', MSYS2_ARG_CONV_EXCL: '*', SYNC_GUI_ASKPASS_SECRET: secret }
      : {
        SSH_ASKPASS: toMsysPath(bash),
        BASH_ENV: '/tmp/sync-gui-askpass.sh',
        SSH_ASKPASS_REQUIRE: 'force',
        DISPLAY: '1',
        SYNC_GUI_ASKPASS_SECRET: secret
      },
...
function sshIdentityPath(identityFile, executable) {
  if (usesNativeWindowsOpenSsh(executable)) return String(identityFile).replace(/\\/g, '/');
  return toMsysPath(identityFile);
}
